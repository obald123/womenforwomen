...
async function getEvent(slug: string) {
  const res = await fetch(`${API_URL}/api/public/events/${slug}`, { cache: "no-store" });
...
export default async function EventDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const badge = item.badgeLabel || (item.isFeatured ? "FEATURED EVENT" : "EVENT");

  return (
    <div className="flex flex-col font-[family-name:var(--font-montserrat)] antialiased bg-white">
      <section className="relative min-h-[55vh] w-full overflow-hidden bg-[#061E1C]">
        <div className="absolute inset-0 z-0">
          ...
          <div className="absolute inset-y-0 left-0 w-[70%] bg-gradient-to-r from-[#05201f]/85 via-[#0A6D66]/45 to-transparent md:w-[55%]" />
        </div>
        ...
        <div className="relative z-10 mx-auto w-full max-w-5xl px-6 pb-14 pt-16 text-white">
          ...
          <div className="mt-6 flex flex-wrap gap-4 text-[12px] uppercase tracking-[0.18em] text-white/85">
            <div className="flex items-center gap-2">
              <Calendar className="h-4 w-4 text-[#00A991]" />
              {formatDate(item.eventDate)}
            </div>
            <div className="flex items-center gap-2">
              <Clock className="h-4 w-4 text-[#00A991]" />
              {item.isOnline ? "Online" : "In person"}
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-[#00A991]" />
              {item.location}
            </div>
          </div>
        </div>
